...
void motor_init(){
    ...
    Location_send.Location_t.acc_flag   = 0xd0;     // 加速度档位：0x00表示0档（无加速度），其他值表示不同的加速度档位
...
void Check_data_from_python(void){
	...
	if(cbuff_state){
//		if(get_vofa[0] == 0xaa && get_vofa[1] == 0x01){//设定移动
//			a = uint8_to_float(get_vofa[2],get_vofa[3],get_vofa[4],get_vofa[5]);
//			Motor_run_(a);
//		
//		else if(get_vofa[0] == 0xaa && get_vofa[1] == 0x02)//改变电机使能，失能
//			a = uint8_to_float(get_vofa[2],get_vofa[3],get_vofa[4],get_vofa[5]);
//			if (a == 0){
//				motor_enable(0);
//			}
