...
void usart2_init(uint32_t bps)
{
    ...
    //dma init
		CircBuf_Init(&USART2_RxCBuf, USART2_RxBuff, RX2_BUF_SIZE);
...
void nvic_config(uint32_t com)
{
    switch (com)
    {
        case EVAL_COM0:
            ...
            nvic_irq_enable(DMA0_Channel4_IRQn, 0, 1); //RX
...
void USART0_IRQHandler(void)
{
    ...
    if(usart_interrupt_flag_get(USART0, USART_INT_FLAG_IDLE))
    {
        ...
        //把DMA数据存到环形缓冲
				...
				//老方法实现
//				if(USART0_RX_CACHE_BUFFER[0] == 0xA5 && USART0_RX_CACHE_BUFFER[1] == 0x5A){
//					
//					
//					memcpy(date_from_py.date_u8, USART0_RX_CACHE_BUFFER, sizeof(date_from_py.date_u8));  // 拷贝实际接收到的数据
//					uart0_flag = 1;
////					memcpy(date_from_py.date_u8,datadadada,sizeof(date_from_py.date_u8));
//					memset(USART0_RX_CACHE_BUFFER,0,sizeof(USART0_RX_CACHE_BUFFER));
//					date_from_py.data_struct.data_len =  (uint16_t)(date_from_py.date_u8[3] << 8) | date_from_py.date_u8[4];
//					date_from_py.data_struct.check_data = (uint16_t)(date_from_py.date_u8[7] << 8) | date_from_py.date_u8[8];
//				}
//				else
//					memset(USART0_RX_CACHE_BUFFER,0,sizeof(USART0_RX_CACHE_BUFFER));
