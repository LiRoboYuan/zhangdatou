...
void send_data(uint8_t *send, int len) {\
		int i = 0;
    for (i; i < len; i++) {
        // 发送当前字节
        USART_SendData(EVAL_COM1, send[i]);

        // 等待数据发送完成，直到 TC 标志位为 SET
...
//				ulResult = ADS_sum( ADS1256_MUXP_AIN0);
        ...
        ldVolutage = ulResult * 0.59604644775390625 / 1000000.0;
				value_send = ldVolutage*10000;
				send[1] = value_send >> 8;         // 提取低字节
				send[2] = value_send;  // 提取次低字节
				send_data(send,3);
//				printf("%.4f ,%d\n",ldVolutage, value_send);
        Delay(1000);
//				time_ms++;
    }
