 ...
 ******************************************************************/
uint32_t get_encoder_speed(encoder_num_enum encoder)
{
...
void set_encoder_currentCount(encoder_num_enum encoder, int16_t current_count)
{
 ...
 * 备       注：无
 ******************************************************************/
int16_t get_encoder_currentCount(encoder_num_enum encoder)
{
    uint32_t ret = 0;
...
void lock_encoder_currentCount(encoder_num_enum encoder)
{
    switch (encoder)
    {
        case MOTO_A:
...
void unlock_encoder_currentCount(encoder_num_enum encoder)
{
    switch (encoder)
    {
        case MOTO_A:
 ...
 * 备       注：无
 ******************************************************************/
void clear_encoder_currentCount(encoder_num_enum encoder)
{
    lock_flag = 0xff;//变量保护
...
void EXTI10_15_IRQHandler(void)
{
    ...
    if (RESET != exti_interrupt_flag_get(ENCODER_B_EXTI_LINE))
    {
		...
		//掉电保存，注意这个地方，如何使用了带有电流保护的电源这个会误触发
