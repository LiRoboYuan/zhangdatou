...
struct {
	...
	uint64_t tick;
	MOTO_STATUS_ENUM status;
	//uint8_t stop_flag;
}moto_lock_project_pra[4] = {0};
...
void set_moto_control(MOTO_NUM moto_num, int32_t speed)
{
    switch(moto_num)
    {
        case MOTO_4:
...
#if 0
//堵转检测
uint8_t check_moto_locked_rotor(MOTO_NUM moto_num,uint32_t encoder_count)
{
		
		uint8_t ret = 0;
		
		if(moto_lock_project_pra[moto_num].encoder_count == encoder_count)
		{	
				if((get_system_tick() - moto_lock_project_pra[moto_num].tick)>= 1000)
				{
					ret = 1;
				}
		}
		...
		{
				moto_lock_project_pra[moto_num].encoder_count = encoder_count;
				moto_lock_project_pra[moto_num].tick = get_system_tick();
		}
...
//堵转检测
uint8_t set_moto_lock_rotor(MOTO_NUM moto_num, int32_t speed, uint32_t encoder_count)
{
		uint8_t ret = 0;
		...
		if(speed > 400)
		{		
				if((moto_lock_project_pra[moto_num].start_flag & 0x01) == 0 )
				{
					moto_lock_project_pra[moto_num].start_flag = 0x01;
					moto_lock_project_pra[moto_num].tick = get_system_tick();
				}
			
				moto_lock_project_pra[moto_num].lock_flag = check_moto_locked_rotor(moto_num,encoder_count);
				
		}
		else if(speed < -400)
		{
				if((moto_lock_project_pra[moto_num].start_flag & 0x02) == 0 )
				{
					moto_lock_project_pra[moto_num].start_flag = 0x02;
					moto_lock_project_pra[moto_num].tick = get_system_tick();
				}
				moto_lock_project_pra[moto_num].lock_flag = check_moto_locked_rotor(moto_num,encoder_count);
		}
		...
		{
				moto_lock_project_pra[moto_num].start_flag = 0;
				moto_lock_project_pra[moto_num].lock_flag = 0;
		}
		
		ret = moto_lock_project_pra[moto_num].lock_flag;
		return ret;
		
}
#else
uint8_t check_moto_status(MOTO_NUM moto_num,int32_t speed,int32_t encoder_count)
{
			if(moto_num >= MOTO_MAX_NUM)
			{
				return STOP;
			}
	
		if(abs(moto_lock_project_pra[moto_num].encoder_count - encoder_count)>=1)
		{
				moto_lock_project_pra[moto_num].tick = get_system_tick();
				moto_lock_project_pra[moto_num].encoder_count = encoder_count;
				moto_lock_project_pra[moto_num].status = RUN;
				return RUN;
		}
		else
		{
		...
		if((get_system_tick() - moto_lock_project_pra[moto_num].tick) >= 1500)
		{
				if(speed > 400)
				{
					moto_lock_project_pra[moto_num].status = POSITIVE_LOCK;
				}
				else if(speed < -400)
				{
					moto_lock_project_pra[moto_num].status = NEGATIVE_LOCK;
				}
				else
				{
					moto_lock_project_pra[moto_num].status = STOP;
				}
					
		}
		
		return moto_lock_project_pra[moto_num].status;
}

void clean_moto_status(MOTO_NUM moto_num)
{
		if(moto_num < MOTO_MAX_NUM)
		{
			moto_lock_project_pra[moto_num].tick = get_system_tick();
...
uint8_t get_moto_run_status(MOTO_NUM moto_num)
{
		if(moto_num >= MOTO_MAX_NUM)
		{
				return STOP;
